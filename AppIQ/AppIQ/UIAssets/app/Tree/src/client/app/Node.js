...
export default class Node extends React.Component {
  ...
  handleMouseIn(event) {
    ...
    this.qtip = d3.tip()
	  ...
	  .direction('se')
      .html(function () {
        var title = "<strong>" + titleMap[d.name] + " Information</strong></br>";
        var htmlstring = "<table>";
        if(d.name != "Node" && d.name != "AppProf" && d.label != "App Unmapped EPs" ) {
          htmlstring = htmlstring + "<tr><td><div class='nodeAttributesBase'>Tiers</div></td><td><div class='nodeAttributesBase'>: " + d.sub_label + "</div></td></tr>";
        }
        Object.keys(d.attributes).map(function (key, index) {
          if (key != "ServiceEndpoints" && key != "HealthRuleViolations" && key != 'Contracts') {
            htmlstring = htmlstring + "<tr><td><div class='nodeAttributesBase'>" + key + "</div></td><td><div class='nodeAttributesBase'>: " + d.attributes[key] + "</div></td></tr>";
          }
        });
        var htmlstring = htmlstring + "</table>";
        if (d.attributes.hasOwnProperty('ServiceEndpoints')) {
          htmlstring = htmlstring + "<table></br></br><strong>Service Endpoints:</strong></br>"
          var i;
          for (i = 0; i < d.attributes.ServiceEndpoints.length; ++i) {
            Object.keys(d.attributes.ServiceEndpoints[i]).map(function (key, index) {
              if (key != "Service Endpoints:") {
                htmlstring = htmlstring + "<tr><td><div class='nodeAttributesBase'>" + key + "</div></td><td><div class='nodeAttributesBase'>: " + d.attributes.ServiceEndpoints[i][key] + "</div></td></tr>";
              }
            });
          }
        }
		...
		if (d.attributes.hasOwnProperty('HealthRuleViolations')) {
          htmlstring = htmlstring + "<table></br></br><strong>Health Rule Violations:</strong></br>"
          var i;
          for (i = 0; i < d.attributes.HealthRuleViolations.length; ++i) {
            Object.keys(d.attributes.HealthRuleViolations[i]).map(function (key, index) {
              if (key != "HealthRuleViolations:") {
                htmlstring = htmlstring + "<tr><td><div class='nodeAttributesBase'>" + key + "</div></td><td><div class='nodeAttributesBase'>: " + d.attributes.HealthRuleViolations[i][key] + "</div></td></tr>";
              }
            });
          }
        }
		...
		if (d.attributes.hasOwnProperty('Contracts')) {
          htmlstring = htmlstring + "<table></br></br><strong>Contracts:</strong></br>"
          var i;
          for (i = 0; i < d.attributes.Contracts.length; ++i) {
            Object.keys(d.attributes.Contracts[i]).map(function (key, index) {
              if (key != "Contracts:") {
                htmlstring = htmlstring + "<tr><td><div class='nodeAttributesBase'>" + key + "</div></td><td><div class='nodeAttributesBase'>: " + d.attributes.Contracts[i][key] + "</div></td></tr>";
              }
            });
          }
        }
    ...
    return (
      ...
      >
        ...
        <text
          ...
          dy={(this.props.name == "EPG") ? "0em" : ".35em"}
        >
        ...
        {this.props.sub_label ?
          <text
            ...
            dy='10'
          >
            {"(" + this.props.sub_label.replace(/([^,]*),.*/, "$1 + ") + " " + ((this.props.sub_label.split(",").length > 1) ? this.props.sub_label.split(",").length - 1: "") + ")"}
          </text> : ""}
