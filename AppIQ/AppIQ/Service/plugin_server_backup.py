...
# global path
# path = "/Users/nilayshah/Desktop/AppD_cisco23lab/Cisco_AppIQTest/Service/credentials.json" # On local desktop
# path = '/home/app/credentials/credentials.json' # On APIC
#     New implementations
...
def login(appd_creds):
    ...
    try:
        ...
        if login_check == 200 or login_check == 201:
            ...
            # login_resp['status_code'] = str(login_check)
            # login_resp['message'] = "Login Failed"
            return login_check
...
def apps():
    try:
        ...
        app_list = []
        for app in appsList:
            temp_dict = {'appProfileName': app['appName'], 'isViewEnabled': app['isViewEnabled'],
                         'health': str(app['appHealth']), 'appId': app['appId']}
            app_list.append(temp_dict)
        dict['app'] = app_list
        return json.dumps({"payload": dict, "status_code": "200", "message": "OK"})
...
def mapping(tenant, appId):
    try:
        ...
        return json.dumps({"payload": mapping_dict, "status_code": "200",
...
def saveMapping(appId, mappedData):
    try:
...
def merge_aci_appd(tenant, appId):
    ...
    try:
        ...
        for aci in aci_data:
            ...
            print "3"
            if aci['EPG'] not in non_merged_ep_dict.keys() and aci['IP'] not in merged_eps:
                non_merged_ep_dict[aci['EPG']] = ["IP: "+str(aci['IP'])+",  Mac: "+aci['CEP-Mac']]
            else:
                if aci['IP'] not in non_merged_ep_dict[aci['EPG']] and aci['IP'] not in merged_eps:
                    non_merged_ep_dict[aci['EPG']].append("IP: "+str(aci['IP'])+",  Mac: "+aci['CEP-Mac'])
        print "4"
        final_non_merged = {}
        if non_merged_ep_dict:
            for key,value in non_merged_ep_dict.items():
                final_non_merged[key] = list(set(value))
    
        fractions = {}
        if total_epg_count:
            for epg in total_epg_count.keys():
                fractions[epg] = str(len(merged_epg_count[epg]))+"/"+str(total_epg_count[epg])
        updated_merged_list = []
        print "5"
        if fractions:
            for key, value in fractions.iteritems():
                for each in merge_list:
                    if key == each['EPG']:
                        each['fraction'] = value
                        each['Non_IPs'] = final_non_merged[key]
                        ...
                        each['Non_IPs'] = final_non_merged[key]
                        updated_merged_list.append(each)
        # pprint.pprint(updated_merged_list)
        print "6"
        return merge_list#,total_epg_count # TBD for returning values
    except Exception as e:
...
def get_details(tenant, appId):
    try:
        ...
        for each in merged_data:
            ...
            details_list.append(
                {'epgName': each['EPG'], 'epgHealth': epg_health, 'endPointName': each['VM-Name'],
                'tierName': each['tierName'],
                'tierHealth': each['tierHealth']})
        return json.dumps({"payload": details_list, "status_code": "200", "message": "OK"})
    except Exception as e:
